...
[X caCode] = rangingCode(0,settings.samplingFreq,samplesPerCode);
% Perform DFT of the C/A code
caCodeFreqDom = conj(fft(caCode));
% Generate 10msec long C/A code
[Y longCaCode] = rangingCode(0,settings.samplingFreq,10*samplesPerCode);
...
for K = settings.acqSatelliteList
    ...
    if (peakSize/secondPeakSize) > settings.acqThreshold
        ...
        %below was wrong and had to be updated for complex signals; for I&Q
        %input data, all points in the FFT are unique
        ...
        if (fftMaxIndex > uniqFftPts) %and should validate using complex data
            if (rem(fftNumPts,2)==0)  %even number of points, so DC and Fs/2 computed
                ...
                acqResults.carrFreq(K+8)  = -fftFreqBinsRev(fftMaxIndex);
            ...
            acqResults.carrFreq(K+8)  = -fftFreqBins(fftMaxIndex);
        end
