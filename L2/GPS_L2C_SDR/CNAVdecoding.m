...
I_P_InputBits = I_P_InputBits(1:evenLen);

% Now threshold the output and convert it to 0 and 1 
I_P_InputBits = (I_P_InputBits < 0);
...
%% CNAV data decoding =======================================================
for bitSync = 1:2
    % The first bit in I_P_InputBits may be G1 or G2 ouput, the bit
    ...
    for i = 1:size(index) % For each occurrence
        ...
        if ((length(decodedBits) - index(i) +1) >=300 )
            ...
            if (~frmError) 
                ...
                [eph, TOW_temp] = ephemeris_L2CM(navBitsBin',eph);
                % Just save for first message. firstSubFrame is the starting 
                % positions of the first message in the input bit stream I_P_InputBits
    ...
    % If the first bit in I_P_InputBits is just G1 output, then quit the loop
    if (firstSubFrame ~= inf)
        break
    end
end % for bitSync = 1:2
