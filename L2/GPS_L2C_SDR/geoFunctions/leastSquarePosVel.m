function [pos,vel, el, az, dop] = leastSquarePosVel(satpos, obsPR, satvel,ObsPRat,settings)
%Function calculates the Least Square Solution.
%
%[pos,vel, el, az, dop] = leastSquarePosL2cM(satpos, obs, satvel,ObsPRat,settings);
%
%   Inputs:
%       satpos      - Satellites positions (in ECEF system: [X; Y; Z;] -
%                   one column per satellite)
%       obs         - Observations - the pseudorange measurements to each
%                   satellite corrected by SV clock error:(已完成卫星钟差校正)
%                   (e.g. [20000000 21000000 .... .... .... .... ....]) 
%       satvel      - Satellites velocity (in ECEF system: [Vx; Vy; Vz;] -
%                   one column per satellite)
%       ObsPRat     - Observations - the pseudorange rate measurements to 
%                   each satellite corrected by SV clock drift error.
%       settings    - receiver settings
%
%   Outputs:
%       pos         - receiver position and receiver clock error 
%                   (in ECEF system: [X, Y, Z, dt]) 
%       vel         - receiver velocity and receiver clock rate error 
%                   (in ECEF system: [Vx, Vy, Vz, df]) 
...
el      = az;
omcPRat = zeros(nmbOfSatellites, 1);
...
for iter = 1:nmbOfIterations
    ...
    for i = 1:nmbOfSatellites
        if iter == 1
            ...
            [az(i), el(i), dist] = topocent(pos(1:3, :), Rot_X - pos(1:3, :)); %#ok<NASGU>
        ...
        omc(i) = ( obsPR(i) - norm(Rot_X - pos(1:3), 'fro') - pos(4) - trop ); 
