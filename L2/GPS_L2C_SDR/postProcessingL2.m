...
% Written by Darius Plausinaitis, Dennis M. Akos, Yafeng Li
...
%% Acquisition ============================================================
    ...
    if ((settings.skipAcquisition == 0) || ~exist('acqResults', 'var'))
        ...
        % (20+X+1) ms of raw signal from the front-end.The first 20+X ms 
        % segment is in order to include at least the first X ms of a CM code;
        % The last 1ms is a margin to make sure the index does not exceeds 
        % matrix dimensions in processing.At least 10 ms signal is used to do 
        % fine acquisition.
        if (settings.acqCohT < 10)
            % Additinal 10ms is needed to do fine acquisition
            samplesLength = round(samplesPerCode/20*(31));
        else
            samplesLength = round(samplesPerCode/20*(20+settings.acqCohT+1));
        end
        
        % Read data for acquisition. 
        data  = fread(fid, dataAdaptCoeff*samplesLength, settings.dataType)';
