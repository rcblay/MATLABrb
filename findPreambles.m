...
% to avoid noise due to tracking loop transients. This value should be
% larger than 40 as seen in line 114
searchStartOffset = 100; 
...
%--- Correct the first subframe of specific Satellites --------------------
% maxSubframe=max(firstSubFrame);
% for channelNr = activeChnList
%     if abs(firstSubFrame(channelNr)-maxSubframe)>5000
%         firstSubFrame(channelNr)=firstSubFrame(channelNr)+6000;
%     end
% end

%--- Conditioning the first subframe to ensure they are pointing to the
%same subframe
minSubframe=min(firstSubFrame(firstSubFrame>0));
for channelNr = activeChnList
    if firstSubFrame(channelNr)-minSubframe>5000
        corr=round((firstSubFrame(channelNr)-minSubframe)/6000);
        firstSubFrame(channelNr)=firstSubFrame(channelNr)-corr*6000;
    end
end