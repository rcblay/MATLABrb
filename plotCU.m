% plotCU plots the same time range of data from CU on the same
% plot.
...
start_time = unixtime([2017 1 9 18 0 0]);
...
title(['CU 1B81 (' datestr(unixtime(start_time)), ' to ', ...
...
if x_tick_location == 0
    ...
    if (tot_time <= 86400)
        while tick_loc(end) < CU_plotted_time(end)
            % If greater than 2 hours, location is spread out every 6 hours
            if( tot_time > 6*3600 ) 
                temp_tick_loc = temp_tick_loc + (6*60*60);
        ...
        while tick_loc(end) < CU_plotted_time(end)
            if( (CU_plotted_time(end)-CU_plotted_time(1)) > 6*3600 )
                temp_tick_loc = temp_tick_loc + (6*60*60);
            else
                temp_tick_loc = temp_tick_loc + (60*60);
            end
